...
int main(int argc, char **argv)
{
  if(argc < 2)
    {
      std::cerr << "usage: stats_bound_backward GAME_NAME [FORWARD_PLY] [BACKWARD_PLY]\n";
      return 1;
    }
  ...
  auto load_result = [&](int ply, std::string result) {
    std::string dfa_name = std::format("bound_backward,forward_ply_max={:03d},backward_ply_max={:03d},ply={:03d},{:s}", forward_ply_max, backward_ply_max, ply, result);
