//! Check that every position in a DFA is terminal and lost for the side to
//! move.  Rust port of `src/verify_lost_sound.cpp`.
...
fn run(args: &Args) -> anyhow::Result<()> {
    ...
    verify::verify_lost_sound(game.as_ref(), side_to_move, &dfa, &args.dfa_name)?;
    Ok(())
}
